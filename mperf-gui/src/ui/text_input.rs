...
pub enum InputEvent {
    Changed,
    /// Escape was pressed while the input had focus; the parent decides
    /// where focus goes and what clears.
    Escaped,
}

/// Single-line shadcn-style input with optional leading icon and clear
/// button. v1 scope: insert/backspace/delete/arrows/home/end/select-all,
/// clipboard, IME composition via `EntityInputHandler`.
...
impl TextInput {
    ...
    /// Shortcut hint rendered as a `Kbd` at the trailing edge while the
    /// input is empty.
        ...
        let (selection, cursor) = if selected_range.is_empty() {
            ...
            (
                Some(fill(
                    ...
                    theme.viz.series[0].opacity(0.3),
                )),
                None,
            )
        };
...
impl Render for TextInput {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .gap(px(6.0))
            .h(px(self.height))
            .when_some(self.width, |el, w| el.w(px(w)))
            .px(px(8.0))
            .rounded(theme.radius_md())
            .border_1()
            .border_color(theme.input)
            .when(theme.dark, |el| el.bg(theme.input.opacity(0.3)))
            .in_focus(|s| s.border_color(theme.ring))
            .text_size(px(self.text_size))
            .text_color(theme.foreground)
            .when_some(self.leading_icon, |el, ic| {
                el.child(icon(ic).size(px(12.0)).color(theme.muted_foreground))
            })
            .child(
                div()
                    .flex_1()
            ...
            .when(!has_content, |el| {
                el.when_some(self.kbd_hint.clone(), |el, hint| el.child(kbd(hint)))
            })
            .when(self.clearable && has_content, |el| {
                el.child(
                    div()
                        .id("clear")
                        .flex()
                        ...
                        .on_click(move |_, _, cx| {
                            entity.update(cx, |input, cx| input.clear(cx));
                        })
